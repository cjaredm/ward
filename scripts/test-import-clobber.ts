 ...
 * import, then asserts everything survived byte-for-byte. Run this before every
...
const householdId = await withClient(async (client) => {
  const parcel = await client.query<{ parcel_id: string }>(
    `SELECT parcel_id FROM parcels ORDER BY parcel_id LIMIT 1`,
  )
  if (parcel.rowCount === 0) throw new Error('No parcels in the database — run `npm run import:parcels` first.')
...
await withClient(async (client) => {
  ...
  const h = await client.query<{
    ...
    `SELECT family_name, status, ministering_district, notes, parcel_id
  ...
  const parcelId = h.rows[0]?.parcel_id
  if (parcelId) {
    const o = await client.query<{ in_ward: boolean; is_residential: boolean; address: string | null }>(
      `SELECT in_ward, is_residential, address FROM parcels WHERE parcel_id = $1`,
      [parcelId],
    )
    ...
    // Sanity: the import did run and did refresh county columns on this row.
    const fresh = await client.query<{ recent: boolean }>(
      `SELECT imported_at > now() - interval '5 minutes' AS recent FROM parcels WHERE parcel_id = $1`,
      [parcelId],
    )
    check('import actually touched this parcel', fresh.rows[0].recent, true)
  }

  // Cleanup.
  await client.query(`DELETE FROM households WHERE id = $1`, [householdId]) // people cascade
  if (parcelId) {
    await client.query(
      `UPDATE parcels SET in_ward = true, is_residential = (address IS NOT NULL AND btrim(address) <> '')
       WHERE parcel_id = $1`,
      [parcelId],
    )
  }
